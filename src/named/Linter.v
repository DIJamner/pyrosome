...
Section WithVar.
  ...
  Section LintRule.
    ...
    Definition check_constr_in_lang n :=
      if inb n l then [] else [constr_unbound_in_lang l n].
    ...
    Section LintTerm.
      ...
      Definition check_fresh_in_lang n :=
        if inb n l then [constr_not_fresh_in_lang l n] else [].
     ...
     TODO: also check term vs sort constr
      ...
      Fixpoint lint_term  (e : term) : list lint_error :=
        ...
        | con n s => (check_constr_in_lang n)++(flat_map lint_term s)
        end.
      ...
      Definition lint_sort (e : sort) : list lint_error :=
        match e with
        | scon n s => (check_constr_in_lang n)++(lint_args s)
        end.
  ...
  Fixpoint lint_lang_ext (l_base l : lang) :=
    ...
    | [] => []
    | (n,r)::l' =>
        (check_fresh_in_lang (map fst (l' ++ l_base)) n)
          ++ (lint_rule (map fst (l' ++ l_base)) r)
