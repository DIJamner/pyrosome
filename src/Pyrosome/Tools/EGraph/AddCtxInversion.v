...
Section WithVar.
  ...
  Section AddCtxInvert.
    ...
    (* by the LHS atoms); [Hrig_skip] records that the skip is gated  *)
    (* on per-variable pattern rigidity [rigid_term_skip l c (con n0 s0) x] *)
    (* (Theory.PatternRigidity).  Under that gate the declared-sort wf *)
    (* of a skipped var is recovered from the whole-LHS image via      *)
    (* [PatternRigidity.covering_var_leaf_rigid_con]; see              *)
    ...
    Lemma eq_ctx_inversion_gen (no_sort : V -> bool) (rf : nat) (a : interp) c e1 t
        ...
        (* the per-variable pattern-rigidity gate; the adapter passes
           [rigid_term_skip l c (con n0 s0)] here (which returns false for
           var-rooted LHS, discharging the bare-var case). *)
        (Hrig_skip : forall x, no_sort x = true -> rigid_term_skip l c e1 x = true)
        (* a bare-var LHS never skips any sort (rigid_term_skip returns false
           for var-rooted e1); this rules out the dead bare-var-with-skip case. *)
    ...
    Proof.
      ...
      destruct HE as (Huf1 & Hdb1 & Hroots1 & Hmapfst & Hok1).
      assert (Hmapfst' : map fst c = map fst sub) by (symmetry; exact Hmapfst).
      (* model-free ctx readback in e_ctx *)
      pose proof (@Theorems.add_ctx_readback_gen V V_Eqb V_Eqb_ok V_default V_map V_map_ok V_trie V_trie_ok succ sort_of lt lt_asymmetric lt_succ lt_trans X HX l Hwf Hsof no_sort c Hwfc) as HRb.
      unfold vc in HRb. specialize (HRb e0).
      fold sub e_ctx in HRb.
      unfold Theorems.ctx_readback_post_gen in HRb.
      specialize (HRb Huf0 Hdb0).
      destruct HRb as (_ & _ & _ & _ & Hrb).
      pose proof (@Theorems.add_open_term_all_roots V V_Eqb V_Eqb_ok V_default V_map V_map_ok
                    V_trie V_trie_ok succ sort_of lt lt_asymmetric lt_succ lt_trans X HX l Hwf Hsof
                    c sub e1 t Hwfe1 Hwfc Hmapfst') as Hroots.
      unfold vc in Hroots. specialize (Hroots e_ctx).
      ...
      specialize (Hroots Huf1 Hdb1 Hroots1).
      fold e_open in Hroots.
      destruct Hroots as (Henv & Hx1_root).
      unfold Theorems.roots_env in Henv.
      destruct Henv as (Huf_open & Hdb_open & Hdb_incl & Hroots_mono).
      assert (Hbase_keys : all (fun p => Sep.has_key (snd p) (parent (Defs.equiv e_ctx))) sub)
        by (eapply all_wkn; [| exact Hroots1];
      ...
      destruct Hnode_term as (_ & Hx1_root' & Htree_open & Hext).
      unfold add_open_term in x1.
      (* lift ctx readback e_ctx -> e_open, then to eF *)
      ...
      { destruct e1 as [ev | n0 s0].
        ...
        - (* con LHS: the root atom is in eF, sound, hence [interprets_to_term]
          ...
          destruct Hwfe1 as (c'' & args'' & t'' & Hin & Hwfa'' & Hsort'').
          inversion Htree_eF as [| nn ss sids xee HF2 Hatom Heqnn Heqxee]; subst.
          ...
          destruct (map.get a x1) as [out|] eqn:Hgx1; cbn beta iota in Hsnd; [|contradiction].
          change (domain V lang_model) with (term + sort)%type in Hsnd.
          cbn [interprets_to lang_model] in Hsnd.
          inversion Hsnd as
          ...
          { exfalso. apply eq_sort_wf_l in Heqs; eauto with lang_core.
            safe_invert Heqs.
            match goal with Hsr : In (n0, sort_rule _ _) l |- _ =>
              pose proof (in_all_fresh_same _ _ _ _ ltac:(eauto with lang_core) Hin Hsr) as Hbad end.
            discriminate Hbad. }
          { exists e_out. solve [exact Hgx1 | reflexivity]. } }
      destruct Hx1_inl as [es1 Hx1_inl].
      assert (Hskipvals : forall x, In x (map fst c) -> no_sort x = true ->
                exists es, map.get a (named_list_lookup default sub x) = Some (inl es)).
      { intros x Hxc Hxns.
        eapply (@Theorems.atom_tree_leaf_inl V V_Eqb V_default V_map V_trie sort_of X l
                  a eF sub Hsound e1 x1 Htree_eF c t Hwfe1 es1 Hx1_inl x).
        apply Hskip; exact Hxns. }
      pose proof (@CtxReadback.ctx_readback_vals_gen V V_Eqb V_Eqb_ok V_default V_map
                    V_trie sort_of X l no_sort eF a Hsound c sub Hwfc
                    Hmapfst' Hrbef Hskipvals) as Hvals0.
      destruct Hvals0 as [sg [Hdomsg Hfaith] ].
      pose proof (@Theorems.atom_tree_to_represents V V_Eqb V_Eqb_ok V_default V_map V_trie sort_of X l Hwf
                    a eF sub sg c Hfaith (eq_sym Hmapfst) e1 t Hwfe1 x1 Htree_eF) as Hrep.
      destruct e1 as [ev | n0 s0].
      ...
      pose proof (@CtxReadback.skip_decl_wf_from_image V V_Eqb V_Eqb_ok V_default V_map
                    V_trie sort_of X l Hwf Hsof no_sort eF a Hsound sg n0 s0 x1 Hrep
                    c sub t Hwfc Hwfe1 Hrig_skip Hmapfst') as Hskipdw.
    ...
    (* [_gen] port over add_ctx_gen ... no_sort. *)
    Lemma eq_assum_sortof_frame_eF_gen (no_sort : V -> bool) (rf : nat) c e1 t
      (Hwfc : wf_ctx l c) (Hwfe1 : wf_term l c e1 t)
      (Hsucc : fst (rebuild rf (snd (add_open_term succ sort_of l false false
                (fst (add_ctx_gen succ sort_of l false false no_sort c (empty_egraph V_default X)))
                e1
    ...
    Proof.
      ...
      assert (Ha_ectx_db : atom_in_db V V V_map V_trie X a (Defs.db e_ctx))
        by (unfold Semantics.atom_in_db in Ha_eopen_db |- *;
            ...
            rewrite <- Hkeeps;
            exact Ha_eopen_db).
      pose proof (@Theorems.assum_sortof_frame_pre_gen V V_Eqb V_Eqb_ok V_default V_map V_map_ok V_trie V_trie_ok succ sort_of lt lt_asymmetric lt_succ lt_trans X HX l Hwf Hsof no_sort c Hwfc) as Hsf_vc.
      unfold vc in Hsf_vc. specialize (Hsf_vc e0).
      fold sub e_ctx in Hsf_vc.
      assert (He0_nosof : forall a0, @Semantics.atom_in_egraph V V V_map V_map V_trie X a0 e0 -> a0.(atom_fn) <> sort_of)
        by (intros a0 Ha0; exfalso;
    ...
    (* [_gen] port over add_ctx_gen ... no_sort.  Skip vars add no db atoms,
       so a non-sort_of atom is either a kept-var's sort node (LEFT) or in
       the e1 tree (RIGHT); the RIGHT branch is where skip vars land. *)
    Lemma eq_assum_db_frame_eF_gen (no_sort : V -> bool) (rf : nat) c e1 t
      (Hwfc : wf_ctx l c) (Hwfe1 : wf_term l c e1 t)
      (Hsucc : fst (rebuild rf (snd (add_open_term succ sort_of l false false
                (fst (add_ctx_gen succ sort_of l false false no_sort c (empty_egraph V_default X)))
                e1
    ...
    Proof.
      ...
      destruct (Hnew_disj a Ha_eopen) as [Ha_ectx | Hnode_eopen].
      - (* LEFT BRANCH: a ∈ e_ctx *)
        pose proof (@Theorems.assum_db_frame_pre_gen V V_Eqb V_Eqb_ok V_default V_map V_map_ok V_trie V_trie_ok succ sort_of lt lt_asymmetric lt_succ lt_trans X HX l Hwf Hsof no_sort c Hwfc) as Hframe_vc.
        unfold vc in Hframe_vc. specialize (Hframe_vc e0).
        fold sub e_ctx in Hframe_vc.
        assert (He0_sortof : forall a0, @Semantics.atom_in_egraph V V V_map V_map V_trie X a0 e0 -> a0.(atom_fn) = sort_of)
          by (intros a0 Ha0; exfalso;
    ...
    (* [_gen] port of good_worklist_eq_assum_sort, over add_ctx_gen. *)
    Lemma good_worklist_eq_assum_sort_gen (no_sort : V -> bool) c t1
    ...
    (* Sort analogue of [eq_ctx_inversion_gen], for the sort_eq query
       ...
       skipped var occurs in the LHS sort [t1]; [Hrig_skip]: the skip is gated
       on per-variable pattern rigidity [rigid_sort_skip l c t1 x]
       (Theory.PatternRigidity).  Discharged the same way as
    ...
    Lemma eq_sort_ctx_inversion_gen (no_sort : V -> bool) (rf : nat) (a : interp) c t1
        ...
        (Hrig_skip : forall x, no_sort x = true -> rigid_sort_skip l c t1 x = true)
      : fst (rebuild rf (snd (add_open_sort succ sort_of l false false
                  (fst (add_ctx_gen succ sort_of l false false no_sort c (empty_egraph V_default X)))
                  t1
                  (snd (add_ctx_gen succ sort_of l false false no_sort c (empty_egraph V_default X))))))
    ...
    Proof.
      ...
      specialize (Hroots Huf1 Hdb1 Hroots1).
      fold e_open in Hroots.
      destruct Hroots as (Henv & Hxs1_root).
      unfold Theorems.roots_env in Henv.
      destruct Henv as (Huf_open & Hdb_open & Hdb_incl & Hroots_mono).
      assert (Hbase_keys : all (fun p => Sep.has_key (snd p) (parent (Defs.equiv e_ctx))) sub)
        by (eapply all_wkn; [| exact Hroots1];
      ...
      cbn [fst snd] in Hnodes_sort.
      fold e_open in Hnodes_sort.
      destruct Hnodes_sort as (_ & Hxs1_root' & Htree_open & Hext).
      pose proof (@Theorems.ctx_readback_gen_mono V V_Eqb V_default V_map V_trie sort_of X no_sort
                    e_ctx e_open sub c Hdb_incl Hroots_mono Hext Hrb) as Hrb_open.
      assert (Hroots_open : all (fun p => is_root V V_map V_trie X e_open (snd p)) sub)
        by (eapply all_wkn; [| exact Hroots1];
            intros p _ Hp; exact (Hroots_mono (snd p) Hp)).
      pose proof (good_worklist_eq_assum_sort_gen no_sort c t1 Hwfc Hwft1) as Hgwl_open.
      fold e0 sub e_ctx e_open in Hgwl_open.
      pose proof (@rebuild_survives_canonical V V_Eqb V_Eqb_ok V_default V_map V_map_ok
                    V_trie V_trie_ok succ sort_of lt X HX l Hwf Hsof
                    e_open rf Hok_open Hgwl_open Hsucc) as Hsurv0.
      ...
      assert (Hsurv : forall a0 : atom,
        ...
        by (intros a0 Ha0_in Ha0_args Ha0_ret;
            ...
            [apply Hrefl_per; exact Ha0_args | apply Relations.PER_clo_base; exact Ha0_ret]).
      pose proof (@Theorems.atom_tree_sort_survives V V_Eqb V_default V_map V_trie sort_of X l Hsof
                    e_open eF sub c Hdb_open Hsurv t1 Hwft1
                    (fst (add_open_sort succ sort_of l false false sub t1 e_ctx)) Htree_open) as Htree_eF.
      pose proof (@CtxReadback.ctx_readback_to_eF_gen V V_Eqb V_default V_map V_trie sort_of X l Hsof no_sort
                    e_open eF Hdb_open Hsurv0 c sub Hwfc Hroots_open Hrb_open) as Hrbef.
      (* skip-var values via [atom_tree_sort_leaf_inl] over the LHS sort tree. *)
      destruct t1 as [n0 s0].
      pose proof Htree_eF as Htree_eF0.
      destruct Htree_eF as (sids & HF2 & Hatom_root).
      assert (Hskipvals : forall x, In x (map fst c) -> no_sort x = true ->
                exists es, map.get a (named_list_lookup default sub x) = Some (inl es)).
      { intros x Hxc Hxns.
        eapply (@Theorems.atom_tree_sort_leaf_inl V V_Eqb V_default V_map V_trie sort_of X l
                  a eF sub Hsound n0 s0 sids _ HF2 Hatom_root c Hwft1 x).
        apply Hskip; exact Hxns. }
      pose proof (@CtxReadback.ctx_readback_vals_gen V V_Eqb V_Eqb_ok V_default V_map
                    V_trie sort_of X l no_sort eF a Hsound c sub Hwfc
                    Hmapfst' Hrbef Hskipvals) as Hvals0.
      destruct Hvals0 as [sg [Hdomsg Hfaith] ].
      assert (Hrep : @Theorems.represents_sort V V_Eqb V_default V_map V_trie sort_of X l a eF sg
                       (scon n0 s0) (fst (add_open_sort succ sort_of l false false sub (scon n0 s0) e_ctx))).
      ...
      pose proof (@CtxReadback.skip_decl_wf_from_image_sort V V_Eqb V_Eqb_ok V_default V_map
                    V_trie sort_of X l Hwf Hsof no_sort eF a Hsound sg n0 s0 _ Hrep
                    c sub Hwfc Hwft1 Hrig_skip Hmapfst') as Hskipdw.
      specialize (Hskipdw Hdomsg (fun x Hx => Hskip x Hx) Hfaith).
      pose proof (@CtxReadback.ctx_readback_wf_subst_gen V V_Eqb V_Eqb_ok V_default V_map
                    V_trie sort_of X l Hwf Hsof no_sort eF a Hsound c sub Hwfc
                    Hmapfst' Hrbef Hskipdw) as Hfin.
    ...
    (* [_gen] sort port. *)
    Lemma eq_sort_add_ctx_readback_eF_gen (no_sort : V -> bool) (rf : nat) c t1
        (Hwfc : wf_ctx l c) (Hwft1 : wf_sort l c t1)
        (Hsucc : fst (rebuild rf (snd (add_open_sort succ sort_of l false false
                  (fst (add_ctx_gen succ sort_of l false false no_sort c (empty_egraph V_default X)))
                  t1
    ...
    (* [_gen] sort port. *)
    Lemma eq_sort_assum_sortof_frame_eF_gen (no_sort : V -> bool) (rf : nat) c t1
      (Hwfc : wf_ctx l c) (Hwft1 : wf_sort l c t1)
      (Hsucc : fst (rebuild rf (snd (add_open_sort succ sort_of l false false
                  (fst (add_ctx_gen succ sort_of l false false no_sort c (empty_egraph V_default X)))
                  t1
    ...
    (* [_gen] sort port. *)
    Lemma eq_sort_assum_db_frame_eF_gen (no_sort : V -> bool) (rf : nat) c t1
      (Hwfc : wf_ctx l c) (Hwft1 : wf_sort l c t1)
      (Hsucc : fst (rebuild rf (snd (add_open_sort succ sort_of l false false
                  (fst (add_ctx_gen succ sort_of l false false no_sort c (empty_egraph V_default X)))
                  t1
