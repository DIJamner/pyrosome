...
Lemma compile_ctx_empty cmp
  : compile_ctx cmp [] = [].
Proof.
  unfold compile_ctx.
  rewrite flat_map_all_empty; auto.
Qed.  
Hint Rewrite compile_ctx_empty : lang_core.
...
Lemma compile_strengthen_term cmp n cc e cname
  : fresh n (snd cmp) ->
    all_fresh (snd cmp) ->
    all_constructors (fun n => In n (map fst (snd cmp))) e ->
    compile (extend_cmp [(n,cc)] cmp) cname e = compile cmp cname e.
Proof.
  revert cname.
  ...
  f_equal.
  f_equal.
  apply flat_map_funext.
  intros x Inx.
  apply map_ext_in.
  intros a Ina.
  pose proof (in_all _ _ H Ina).
  simpl in *.
  apply H2; auto.
  eapply in_all; eauto.
Qed.
...
Lemma compile_strengthen_args cmp n cc e
  : fresh n (snd cmp) ->
    all_fresh (snd cmp) ->
    all (all_constructors (fun n => In n (map fst (snd cmp)))) e ->
    compile_args (extend_cmp [(n,cc)] cmp) e = compile_args cmp e.
Proof.
  induction e; basic_goal_prep; basic_core_crush.
...
Proof using.
    ...
    basic_core_crush.
  {
    ...
    case_match; basic_core_firstorder_crush;
      pose proof (lang_compiler_sort_case_args_eq _ _ _ _ _ H H0 H2 HeqH4);
      basic_core_firstorder_crush.
    {
      fold_Substable.
      match goal with
  ...
  }
  {
    case_match; basic_core_crush.
    case_match; basic_core_firstorder_crush.
    pose proof (lang_compiler_term_case_args_eq _ _ _ _ _ _ H H0 H2 HeqH4);
      basic_core_firstorder_crush.
    {
      fold_Substable.
      match goal with
