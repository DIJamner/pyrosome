...
(*                                                                         *)
(* RESOLUTION (2026-06-05): the app clause is now GATED BY RENAMINGS.         *)
(* A prior note feared (4b) needed the full Fundamental Lemma: under an        *)
(* ARBITRARY well-typed [sg], [n[sg]] need not stay neutral, so [body[a::sg]] *)
(* is a real beta-normal value, reducible only by the FL's semantic-          *)
(* application closure -- with which it would be mutually entangled.  But      *)
(* that entanglement was an artifact of quantifying the clause over arbitrary  *)
(* [wf_ssub].  Quantifying over an arbitrary [wf_ssub] is in fact UNSOUND for  *)
(* an eta-expansion (a non-reducible well-typed [sg] makes [body[a::sg]]       *)
(* non-reducible), and a reducible-substitution hypothesis cannot live inside  *)
(* [LR] (negative occurrence -> strict positivity).  The positivity-safe and   *)
(* sound fix, as in logrel-coq, is the RENAMING gate [rn : is_ren sg] now      *)
(* carried by [PiRedTmPred] (LogRel.v): a renaming of a neutral STAYS NEUTRAL  *)
(* ([LogRelRen.ren_Apply_total]), so [n[sg]] is neutral and [body[a::sg]] is   *)
(* again a reflection at the codomain, reducible by the [posAd] codomain IH.   *)
(*                                                                         *)
(* What remains for (4b) is therefore the REFLECT/REIFY adequacy core:         *)
(* relating [body[up sg][a::id]] to the reflection of the applied neutral      *)
(* [nApp (n[sg]) (reify a)] at [posTy PA ra] (via [Reflect] naturality under   *)
(* renamings + [Reflect_det]), then closing with the codomain reflect IH.      *)
(* Note [a] is reducible-but-not-necessarily-eta-long (it may be a reducible   *)
(* neutral at a Pi domain), so the bridge is UP-TO-LR via reify, not a         *)
(* syntactic [ARG[a::sg] = a] identity.  General reducible substitutions live  *)
(* in the separate validity layer (VR), where the FL is proved.                *)
(* ===================================================================== *)
