(* =====================================================================
   Closed-substitution context reframing (the "c'[/s/]" infrastructure).

   For a CLOSED substitution [s] (every value well-scoped in []), the
   pointwise context substitution [ctx_subst s c'] has CLOSED sorts.  Over
   such a context, [wf_subst l [] s -] is a FLAT pointwise condition -- no
   telescope/tail-subst coupling -- and is PERMUTATION-INVARIANT.

   Main results (all Qed, axiom-free):
   - [claim1_fwd] / [claim1_bwd]:
       wf_subst l [] s c'  <->  wf_subst l [] s (ctx_subst s c').
   - [claim2_fwd] / [claim2_bwd]:
       over a closed ctx, wf_subst <-> flat pointwise wf at the declared sorts.
   - [wf_subst_closed_perm]:
       wf_subst over a closed ctx is invariant under reordering the context.

   These are reusable substitution-calculus facts.  NOTE (min_sorts_query):
   they do NOT break the reflection wall.  By [claim1], building
   [wf_subst l [] s (ctx_subst s c')] is EQUIVALENT to building
   [wf_subst l [] s c'], so the flat form is no easier to CONSTRUCT; it is
   only easier to REORDER once built.  The var-arg leaf still needs to
   transport the source use=declared equation [eq_sort l c' t1 t'] by [s],
   which is [eq_subst l [] c' s s] = the full [wf_subst].  See the
   memory note and [WIP/ReflClosedC0.v] for the concrete leaf dump.
   ===================================================================== *)
