...
Abort.
  ...
  *)
  ...
  Proof.
    ...
    pose proof (empty_sound_for_interpretation m) as Hpair.
    destruct Hpair as [Hok Hsound].
    split; [exact Hok|].
    exists map.empty.
    exact Hsound.
  Qed.
  ...
  *)
  ...
  Lemma pull_worklist_sound Pre
    : state_sound_for_model Pre
        ...
        (fun wl i => Pre i /\ all (worklist_entry_sound m i) wl).
  Proof.
    clear idx_zero idx_succ.
    ...
    { intros j. split.
      - intros [Hwf Hex Ha Hr]; constructor; cbn in *; auto.
      - intros [Hwf Hex Ha Hr]; constructor; cbn in *; auto. }
    split.
    ...
    { econstructor.
      ...
      - intros j Hj.
        pose proof Hj as Hj_orig.
        apply (proj2 (Hiff j)) in Hj_orig.
        split.
        { split; [apply HPre; exact Hj_orig|].
          destruct Hj as [Hwf_j Hex_j Ha_j Hrel_j].
          destruct Hok as [_ Hwl_o _].
          cbn in Hwl_o.
          eapply all_wkn; [|exact Hwl_o].
  ...
  Proof.
    ...
    state_sound_constructor.
    cbn in *.
    specialize (Hc e (conj Hok (Build_forall_nonempty _ _ _ iSSC HiSSC HPre))).
    destruct (c e) as [a e'] eqn:Hce.
    cbn in *.
    destruct Hc as (Hok' & HPost1Ext).
    destruct HPost1Ext as [j1 Hj1 HPost1Ext_all].
    pose proof (HPost1Ext_all _ Hj1) as [HPost1_j1 _].
    specialize (Hf a e' (conj Hok'
                           (Build_forall_nonempty _ _ _ j1 Hj1
                              (fun j Hj => proj1 (HPost1Ext_all j Hj))))).
    destruct Hf as (Hok'' & HPost2Ext).
    split; [exact Hok''|].
    destruct HPost2Ext as [j2 Hj2 HPost2Ext_all].
    econstructor; [exact Hj2|].
    intros j Hj.
    pose proof (HPost2Ext_all j Hj) as Hpall_j.
    destruct Hpall_j as (HPost2_j & i_mid & Hi_mid & Hext_mid).
    split; [exact HPost2_j|].
    pose proof (HPost1Ext_all i_mid Hi_mid) as Hpall_mid.
    destruct Hpall_mid as (_ & i_old & Hi_old & Hext_old).
    exists i_old; split; [exact Hi_old | eauto using map_extends_trans].
  Qed.
  ...
  Proof.
    ...
    specialize (Hc e).
    cbn in Hc.
    destruct Hc as (Hok' & HPostExt).
    { split; [exact Hok|].
      econstructor; [exact HiSSC|]. exact HPre. }
    split; [exact Hok'|].
    destruct HPostExt as [j' Hj' HPostall].
    econstructor; [exact Hj'|].
    intros j Hj.
    pose proof (HPostall j Hj) as [HPost1_j Hext_j].
    split; [apply Himpl; exact HPost1_j | exact Hext_j].
  Qed.

  
  Lemma state_sound_for_model_ret A Pre  (v : A)
    : state_sound_for_model Pre (Mret v) (fun x i => Pre i /\ x = v).
  Proof.
    clear idx_succ idx_zero.
    state_sound_constructor.
    cbn [fst snd].
    ...
    intros j Hj.
    split.
    - split; [apply HPre; exact Hj | reflexivity].
    - exists j; split; [exact Hj | apply Properties.map.extends_refl].
  Qed.
  ...
  Lemma state_sound_for_model_frame {A} (Pre Pre' : idx_map (domain m) -> Prop)
    (c : _ A) Post
    : (forall i, Pre i -> Pre' i) ->
      (forall i i', map.extends i' i -> Pre' i -> Pre' i') ->
      state_sound_for_model Pre c (fun x i => Pre' i -> Post x i) ->
      state_sound_for_model Pre c Post.
  Proof.
    clear idx_succ idx_zero.
    intros Himpl Hmono Hc.
    state_sound_constructor.
    specialize (Hc e (conj Hok (Build_forall_nonempty _ _ _ iSSC HiSSC HPre))).
    destruct Hc as (Hok' & HPostExt).
    split; [exact Hok'|].
    ...
    econstructor; [exact Hj'|].
    intros j Hj.
    pose proof (HPostall j Hj) as Hpall_j.
    destruct Hpall_j as (HPost_j & i_old & Hi_old & Hext).
    split.
    { apply HPost_j.
      eapply Hmono; [exact Hext|].
      apply Himpl. apply HPre. exact Hi_old. }
    { exists i_old; split; [exact Hi_old | exact Hext]. }
  Qed.
  ...
  Lemma state_sound_for_model_Mmap A B
    (Pre P_const : idx_map (domain m) -> Prop)
    (P_elt : idx_map (domain m) -> B -> Prop) l (f : A -> _ B)
    : (forall (a:A),
          state_sound_for_model (fun i => In a l /\ P_const i) (f a)
            (fun b i => P_const i /\ P_elt i b)) ->
      (forall i, Pre i -> P_const i) ->
      (forall b i i', map.extends i' i -> P_elt i b -> P_elt i' b) ->
      state_sound_for_model Pre (list_Mmap f l)
        (fun l' i => P_const i /\ all (P_elt i) l').
  Proof.
    clear idx_succ idx_zero.
    intros Hf HPre_imp Hmono.
    eapply state_sound_for_model_strengthen_pre; [|exact HPre_imp].
    clear Pre HPre_imp.
    (* Generalize the [l] in [list_Mmap f l] vs. the [l] used as the
       fixed index set in the [In a l] hypothesis. *)
    enough (Hgen : forall (l_iter : list A),
               incl l_iter l ->
               state_sound_for_model P_const (list_Mmap f l_iter)
                 (fun l' i => P_const i /\ all (P_elt i) l')).
    { apply Hgen. apply incl_refl. }
    intros l_iter Hincl.
    induction l_iter as [| a l_rest IH].
    - cbn [list_Mmap].
      eapply state_sound_for_model_wkn_post.
      { apply state_sound_for_model_ret. }
      intros r i [HPc Heq]; subst r; split; [exact HPc | exact I].
    - cbn [list_Mmap].
      apply incl_cons_inv in Hincl as [Hin Hincl'].
      eapply state_sound_for_model_bind.
      { eapply state_sound_for_model_strengthen_pre; [apply Hf|].
        intros i HPc. split; [exact Hin | exact HPc]. }
      intros b.
      (* Carry P_elt i b through the recursion, using monotonicity. *)
      unfold state_sound_for_model, state_triple. intros e1 [Hok1 Hne1_pair].
      destruct Hne1_pair as [iSSC1 HiSSC1 HPp1].
      pose proof (HPp1 _ HiSSC1) as [HPc_init HPe_init].
      pose proof (IH Hincl' e1
                    (conj Hok1
                       (Build_forall_nonempty _ _ _ iSSC1 HiSSC1
                          (fun j Hj => proj1 (HPp1 j Hj))))) as IHapp.
      cbn [Mbind StateMonad.state_monad fst snd] in *.
      destruct (list_Mmap f l_rest e1) as [bs e2] eqn:Hbs.
      cbn in IHapp.
      destruct IHapp as (Hok2 & HPostExt12).
      cbn [Mret StateMonad.state_monad fst snd].
      split; [exact Hok2|].
      destruct HPostExt12 as [j' Hj' HPpall].
      econstructor; [exact Hj'|].
      intros j Hj.
      pose proof (HPpall j Hj) as Hpall_j.
      destruct Hpall_j as ((HPcj & Hallj) & i_old & Hi_old & Hext).
      split.
      { split; [exact HPcj|].
        split; [|exact Hallj].
        eapply Hmono; [exact Hext|].
  ...
  Lemma state_sound_for_model_Mmap_dep A B
    (Pre P_const : idx_map (domain m) -> Prop)
    (P_elt : idx_map (domain m) -> A -> B -> Prop) l (f : A -> _ B)
    : (forall (a:A),
          state_sound_for_model (fun i => In a l /\ P_const i) (f a)
            (fun b i => P_const i /\ P_elt i a b)) ->
      (forall i, Pre i -> P_const i) ->
      (forall a b i i', map.extends i' i -> P_elt i a b -> P_elt i' a b) ->
      state_sound_for_model Pre (list_Mmap f l)
        (fun l' i => P_const i /\ all2 (P_elt i) l l').
  Proof.
    clear idx_succ idx_zero.
    intros Hf HPre_imp Hmono.
    eapply state_sound_for_model_strengthen_pre; [|exact HPre_imp].
    ...
    enough (Hgen : forall (l_iter : list A),
               incl l_iter l ->
               state_sound_for_model P_const (list_Mmap f l_iter)
                 (fun l' i => P_const i /\ all2 (P_elt i) l_iter l')).
    { apply Hgen. apply incl_refl. }
    intros l_iter Hincl.
    induction l_iter as [| a l_rest IH].
    - cbn [list_Mmap].
      eapply state_sound_for_model_wkn_post.
      { apply state_sound_for_model_ret. }
      intros r i [HPc Heq]; subst r; cbn; split; [exact HPc | exact I].
    - cbn [list_Mmap].
      apply incl_cons_inv in Hincl as [Hin Hincl'].
      eapply state_sound_for_model_bind.
      { eapply state_sound_for_model_strengthen_pre; [apply Hf|].
      ...
      split.
      { split; [exact HPcj|].
        cbn. split; [|exact Hallj].
        eapply Hmono; [exact Hext|].
        apply (HPp1 _ Hi_old). }
      { exists i_old; split; [exact Hi_old | exact Hext]. }
  Qed.

  Lemma state_sound_for_model_Miter A B
    (Pre : idx_map (domain m) -> Prop)
    l (f : A -> state instance B)
    (P_inv : unit -> idx_map (domain m) -> Prop)
    : (forall (a:A),
          state_sound_for_model (fun i => In a l /\ P_inv tt i) (f a)
            (fun _ => P_inv tt)) ->
      (forall i, Pre i -> P_inv tt i) ->
  ...
  Proof.
    ...
    enough (Hgen : forall (l_iter : list A),
               incl l_iter l ->
               state_sound_for_model (P_inv tt) (list_Miter f l_iter) P_inv).
    { apply Hgen. apply incl_refl. }
    intros l_iter Hincl.
    induction l_iter as [| a l_rest IH].
    - cbn [list_Miter].
      eapply state_sound_for_model_wkn_post.
      { apply state_sound_for_model_ret. }
      intros r i [HPc _]; destruct r; exact HPc.
    - cbn [list_Miter].
      apply incl_cons_inv in Hincl as [Hin Hincl'].
      eapply state_sound_for_model_bind.
      { eapply state_sound_for_model_strengthen_pre; [apply Hf|].
      ...
      intros b.
      eapply state_sound_for_model_wkn_post.
      { apply IH; exact Hincl'. }
      intros r i HPc; exact HPc.
  Qed.
  ...
  Lemma find_sound (Pre : idx_map (domain m) -> Prop) x
    : (forall i, Pre i -> Sep.has_key x i) ->
  ...
  Proof.
    ...
    { apply Hexact. unfold Is_Some, Sep.has_key in *.
      destruct (map.get iSSC x); tauto. }
    destruct Hg_eq as [l Huf_l].
    unfold find.
    destruct (UnionFind.find e.(equiv) x) as [uf' i0] eqn:Hfind.
    cbn [fst snd].
    assert (lt_trans_nat : forall a b c : nat, a < b -> b < c -> a < c)
      by (intros; Lia.lia).
    ...
    | |- _ /\ forall_nonempty (fun i => egraph_sound_for_interpretation m i ?inst) _ =>
        assert (Hiff : forall j, (egraph_ok e /\ egraph_sound_for_interpretation m j e)
                                <-> (egraph_ok inst /\ egraph_sound_for_interpretation m j inst));
          [ intros j;
            apply (egraph_sound_for_interpretation_iff_equiv j e inst l);
            cbn; auto |]
    end.
    ...
    intros j Hj.
    pose proof (proj2 (Hiff j) (conj Hok_new Hj)) as [_ Hj_e].
    split.
    { split; [apply HPre; exact Hj_e|].
      destruct Hj as [Hwf_j Hexact_j Hatom_j Hrel_j].
      apply Hrel_j.
      cbn.
      unfold uf_rel_PER.
  ...
  Lemma canonicalize_worklist_entry_sound (Pre : idx_map (domain m) -> Prop) a
    : (forall i, Pre i -> worklist_entry_sound m i a) ->
      state_sound_for_model Pre
        (canonicalize_worklist_entry idx Eqb_idx symbol
           symbol_map idx_map idx_trie
           analysis_result a)
        (fun a' i => Pre i /\ worklist_entry_sound m i a').
  Proof.
    intros HPa.
    ...
    - eapply state_sound_for_model_wkn_post.
      { apply state_sound_for_model_ret. }
      intros a' i Hp. destruct Hp as [HPre Heq]. subst a'.
      split; [exact HPre|]. cbn; exact I.
  Qed.

  Lemma get_parents_sound (Pre : idx_map (domain m) -> Prop) old_idx
  ...
  Lemma remove_parents_sound (Pre : idx_map (domain m) -> Prop) old_idx
    : state_sound_for_model Pre
  ...
  Proof.
    ...
    econstructor; [exact HiSSC_new|].
    intros j Hj.
    pose proof (proj2 (proj2 (Hiff j) (conj Hok_new Hj))) as Hj_e.
    split.
  ...
  Proof.
    ...
    { split.
      { econstructor; [exact HiSSC_new|].
        intros i Hi.
        pose proof (proj2 (proj2 (Hiff i) (conj Hok_new Hi))) as Hi_old.
        split; [apply HPre; exact Hi_old|].
        unfold ps, unwrap_with_default.
        destruct (map.get parents_e old_idx) as [s'|] eqn:Hg_par; cbn.
        - eapply parents_interpretation with
            (e := Build_instance _ _ _ _ _ _ db_e equiv_e parents_e epoch_e wl_e analyses_e log_e)
            (i := i); eauto.
        - exact I. }
      { unfold ps, unwrap_with_default.
    ...
    { econstructor; [exact HiSSC_new|].
      ...
      split; [exact (proj2 (proj2 (Hiff i') (conj Hok_new Hi'))) |
              apply Properties.map.extends_refl]. }
  Qed.
  ...
  (* [update_entry a] from a state [e] that is the result of
     ...
     admitted here. *)
  (* The full structural lemma for the second half of [repair_each]:
     after [db_remove a] (relating the input state [e] to a
     pre-removal reference state [e_ref] via [post_db_remove]),
     [canonicalize a >>= update_entry] restores the egraph so that
     [Pre], the original [ne_set_maps_to] reference, and the
     [atom_in_egraph_up_to_equiv]-membership of an arbitrary side
     list [side_l] are all preserved.  The proof is the deep
     structural argument (case-split on [update_entry]'s branches
     and use [HPa]+[atom_in_egraph_up_to_equiv a e_ref] to show
     [denote] is preserved up to set inclusion, plus the parents-
     disjunction structural reasoning); it is the same kind of
     argument that is admitted in [db_set_entry_sound']. *)
  ...
  Proof.
    ...
    (* Note on file layout: [state_triple_bind], [fields_preserved],
       and [canonicalize_preserves_fields] are all defined LATER in
       this file (lines ~3037, ~3140, ~3184).  We therefore unfold
       [state_triple] directly and inline the relevant equalities
       rather than forward-referencing those bundlings. *)
    unfold state_triple. intros e Hpost_e.
    cbn beta.
    (* Step 1.  Compute [canonicalize a e].  The result [(a', e1)]
       has the following structural relationship to [(a, e)]
       (the conjuncts of [fields_preserved e e1] plus canonical
       equivalence of [a'] to [a] in [e1.(equiv)]). *)
    unfold Mbind, StateMonad.state_monad at 1.
    destruct (canonicalize a e) as [a' e1] eqn:Hcan.
    cbn [fst snd].
    assert (Hcan_facts :
              e1.(db) = e.(db)
              /\ e1.(parents) = e.(parents)
              /\ e1.(epoch) = e.(epoch)
              /\ e1.(worklist) = e.(worklist)
              /\ e1.(analyses) = e.(analyses)
              /\ (forall y, Sep.has_key y e.(equiv).(parent)
                            <-> Sep.has_key y e1.(equiv).(parent))
              /\ iff2 (uf_rel_PER _ _ _ e.(equiv))
                      (uf_rel_PER _ _ _ e1.(equiv))
              /\ atom_fn a' = atom_fn a
              /\ uf_rel_PER _ _ _ e1.(equiv)
                   (atom_ret a') (atom_ret a)
              /\ all2 (uf_rel_PER _ _ _ e1.(equiv))
                   (atom_args a') (atom_args a)).
    { (* SUB-ADMIT 1: structural facts about [(a', e1) := canonicalize a e].

         Reduction.  Combine [canonicalize_preserves_fields] (later
         in this file; gives the seven [fields_preserved] conjuncts)
         with the [find_spec] used inside each [find] call of
         [canonicalize] (gives the [atom_fn] equality and the
         [uf_rel_PER] facts about [a']).

         To invoke [canonicalize_preserves_fields] we need:
           - [exists l, union_find_ok lt e.(equiv) l]
             — from [Hok_ref] and [Hpost_e ⇒ e.(equiv) =
               e_ref.(equiv)];
           - [all (fun i => Sep.has_key i e.(equiv).(parent))
                a.(atom_args)] and
             [Sep.has_key a.(atom_ret) e.(equiv).(parent)]
             — from [Hne_ref] (giving an interpretation [i]
               satisfying [denote e_ref i]), [HPa i] (giving
               [atom_sound_for_model m i a]),
               [atom_sound_args_have_key] /
               [atom_sound_ret_has_key] (lifting soundness to
               [Sep.has_key x i]),
               [interpretation_exact] (lifting keys-of-[i] to keys
               of [e_ref.(equiv).(parent)]), and [Hpost_e ⇒
               e.(equiv) = e_ref.(equiv)].

         A routine ~100-line lifting; orthogonal to the deep
         semantic argument. *)
      admit. }
    destruct Hcan_facts as (Hdb_eq & Hpar_eq & Hep_eq & Hwl_eq
                            & Han_eq & Hkey_iff & HPER_iff
                            & Hfn_a' & Hret_a'_a & Hargs_a'_a).
    (* Step 2.  Run [update_entry a' e1].  [db_lookup] is read-only
       on state, so the case-split is on its option result. *)
    unfold update_entry, Mbind, StateMonad.state_monad,
           db_lookup, Mret, Mseq.
    cbn beta.
    destruct (map.get e1.(db) a'.(atom_fn)) as [tbl|] eqn:Htbl;
      [ destruct (map.get tbl a'.(atom_args)) as [entry|] eqn:Hentry | ];
      cbn beta iota.
    - (* [Some entry.(entry_value)] — [update_entry] takes the
         [union r (atom_ret a')] branch. *)
      (* SUB-ADMIT 2 (union branch).

         Goal: from state [e1], after [union r (atom_ret a')] (where
         [r := entry.(entry_value)]), establish:
           - [egraph_ok (snd res)]
           - [forall_ne i | denote (snd res) i, Pre i]
           - [ne_set_maps_to (denote e_ref) (denote (snd res))]
           - [all (fun a' => atom_in_egraph_up_to_equiv a' (snd res))
                  side_l]

         Semantic content.  By [Hentry] and [Hfp.db = e0.db] (from
         [fields_preserved e0 e1]) and [post_db_remove e_ref a e0]
         (which says [atom_in_egraph x e0 ↔ atom_in_egraph x e_ref ∧
         (atom_fn x, atom_args x) ≠ (atom_fn a, atom_args a)]), the
         atom [Build_atom (atom_fn a') (atom_args a') r] is in
         [e_ref.(db)] AND distinct from [a] in (fn, args).  By
         [atom_interpretation] (under any [denote e_ref i]),
         [r] interprets to [interprets_to (atom_fn a') (interp args
         of a')] applied to its model arguments.  By [HPa i] +
         [Hargs_a'_a] + [Hret_a'_a] + [rel_interpretation] (lifting
         uf-equivalence to interpretation-equivalence in any
         [denote e_ref i]), the same equation holds for [atom_ret
         a'], so [r] and [atom_ret a'] interpret to the same value.
         Thus [union r (atom_ret a')] only equates idxs that are
         already interpretation-equivalent: [Pre] is preserved.

         Structural content.  [union] pushes a [union_repair] entry
         to the worklist; the entry is [worklist_entry_ok] in the
         post-state because the union literally puts the two args
         into the same root.  [union] merges the equiv classes
         (preserves [parents_ok] and the
         [atom_in_egraph_up_to_equiv]-membership of [side_l] up to
         the now-coarser equivalence — [atom_canonical_equiv] is
         monotone in [equiv]).  [db] is unchanged, so atom membership
         in the db is unchanged.

         The full proof reduces to [union_sound] (currently at the
         [state_sound_for_model m i] level, line 2328) lifted to
         [state_triple] form, plus the [side_l]-preservation
         reasoning (which is monotone in [equiv]: enlarging [equiv]
         only enlarges the [atom_canonical_equiv]-class). *)
      admit.
    - (* [None] (table present, no entry for these args) —
         [update_entry] takes the [db_set a'] branch. *)
      (* SUB-ADMIT 3 (db_set branch, table-present case).

         Goal: from state [e1], after [db_set a'], establish the
         four conjuncts of the postcondition.

         This is the [db_set_entry_sound']-style obligation.  The
         crucial observation: by [Hatom_ref] there exists [aa]
         canonically equivalent to [a] with [aa] literally in
         [e_ref.(db)].  By [post_db_remove e_ref a e0], the only
         entry removed from [e_ref.(db)] in passing to [e0.(db)] is
         the one keyed by [(atom_fn a, atom_args a)].  By [Hfp],
         [e1.(db) = e0.(db)].  So either:
           (i)  [aa] is still in [e1.(db)] — that happens when
                [(atom_fn aa, atom_args aa) ≠ (atom_fn a, atom_args a)].
                But by [atom_canonical_equiv], [atom_fn aa = atom_fn
                a]; so [atom_args aa ≠ atom_args a] (yet
                uf-equivalent).  The [Hentry = None] hypothesis says
                no LITERAL entry for [(atom_fn a', atom_args a')]
                exists in [e1.(db)], so [atom_args a'] also differs
                from [atom_args aa] literally.  This branch is
                pre-canonicalization slack; the proof must still
                handle it.
           (ii) [aa = a] literally, so the removed entry was the only
                witness; [db_set a'] re-installs a witness, this time
                at the canonical args/ret.

         In either subcase, the new entry [Build_atom (atom_fn a')
         (atom_args a') (atom_ret a')] is sound under [denote e_ref
         i] because [a'] is uf-equivalent to [a] (by [Hargs_a'_a] /
         [Hret_a'_a]) and [a] is sound by [HPa].

         Structural content.  [db_set] adds parent entries pointing
         to [a'] for each idx in [atom_ret a' :: atom_args a'];
         these satisfy [atom_in_egraph_up_to_equiv] in the new state
         because [a'] is now literally in the db.  The new analyses
         entries are [analysis_repair]s on the worklist; trivially
         [worklist_entry_ok].  [side_l] atoms remain
         [atom_in_egraph_up_to_equiv] in the new state because
         adding a db entry only enlarges [atom_in_egraph], hence
         [atom_in_egraph_up_to_equiv].

         This is the same shape of admit currently in
         [db_set_entry_sound'] (line 3581).  Discharging it requires
         the per-conjunct semantic preservation argument lifted from
         the simpler [db_set_sound] / [db_set_entry_sound] (the
         single-interpretation versions, already proven). *)
      admit.
    - (* [None] (no table for [atom_fn a']) — [update_entry] takes
         the [db_set a'] branch.  Same shape of obligation as the
         previous case. *)
      (* SUB-ADMIT 3' (db_set branch, no-table case).

         Identical obligation and reduction to SUB-ADMIT 3.  The
         only difference is that the precondition for the [db_set]
         step says no table for [atom_fn a'] exists in [e1.(db)] at
         all (rather than a table present with no entry for these
         args).  In subcase (i) of the analysis above, the witness
         [aa] would have to be in some other table than [atom_fn
         a'], which contradicts [atom_canonical_equiv aa a]
         requiring [atom_fn aa = atom_fn a = atom_fn a']; so
         subcase (i) is vacuous here, and only subcase (ii) [aa = a]
         survives — making this case strictly simpler. *)
      admit.
  Admitted.
  ...
  Lemma canonicalize_sound (Pre : idx_map (domain m) -> Prop) a1
    : (forall i, Pre i -> atom_sound_for_model m i a1) ->
      state_sound_for_model Pre (canonicalize a1)
        (fun a i => Pre i /\ eq_atom_in_interpretation i a a1).
  Proof.
    intros HPa.
    ...
    { eapply state_sound_for_model_Mmap_dep
        with (P_const := Pre)
             (P_elt := fun i arg arg' => eq_sound_for_model m i arg arg').
      - intros arg.
        eapply state_sound_for_model_wkn_post.
        { eapply state_sound_for_model_strengthen_pre with
            (Pre1 := fun i => In arg args1 /\ Pre i).
          { apply find_sound.
            intros i [Hin HPi].
            specialize (HPa _ HPi).
            cbn in HPa.
            ...
            { subst; auto. }
            { apply IH; auto. } }
          intros i [Hin HPi]; split; [exact Hin | exact HPi]. }
        intros r i HP. destruct HP as [HP1 Heq]. destruct HP1 as [Hin HPi].
        split; [exact HPi | exact Heq].
      - intros i HPi; exact HPi.
      - intros arg arg' i i' Hext Heq.
        unfold eq_sound_for_model, Is_Some_satisfying in *.
        destruct (map.get i arg) eqn:Hgarg; try contradiction.
        destruct (map.get i arg') eqn:Hgarg'; try contradiction.
        apply Hext in Hgarg, Hgarg'.
        rewrite Hgarg, Hgarg'. cbn. exact Heq. }
    intros args1'.
    eapply state_sound_for_model_bind.
    { eapply state_sound_for_model_strengthen_pre with
        (Pre1 := fun i => Pre i /\ all2 (eq_sound_for_model m i) args1 args1').
      { apply find_sound.
        intros i [HPi _]. specialize (HPa _ HPi). cbn in HPa.
        eapply atom_sound_ret_has_key in HPa. exact HPa. }
      intros i [HPi Hall]; split; [exact HPi | exact Hall]. }
    intros o'.
    eapply state_sound_for_model_wkn_post.
    { apply state_sound_for_model_ret. }
    intros a' i Hp.
    destruct Hp as [Hpost_bind Heq]. subst a'.
    destruct Hpost_bind as [HPre_args Hretsound].
    destruct HPre_args as [HPi Hargsounds].
    split; [exact HPi|].
...
*)
  ...
  Lemma db_lookup_entry_sound (Pre : idx_map (domain m) -> Prop) f args
    : state_sound_for_model Pre
               ...
               <-> (egraph_ok
                    ...
                    /\ egraph_sound_for_interpretation m j
                         (Build_instance _ _ _ _ _ _ db_e equiv_e parents_e epoch_e
                            wl_e analyses2 log_e))).
    { intros j a1 a2.
      split; intros [Hok' Hsnd'];
        destruct Hsnd' as [Hwf Hex Ha Hr];
        destruct Hok' as [Heq Hwl Hpa];
        (split; [constructor; cbn; auto | constructor; cbn in *; auto]). }
  ...
  Proof.
    ...
    { (* None case: update_analyses then push_worklist *)
      ...
      cbn in Hupd.
      destruct Hupd as (Hok1 & HPostExt1).
      destruct HPostExt1 as [j1 Hj1 HPostall1].
      ...
      cbn in Hpw.
      destruct Hpw as (Hok2 & HPostExt2).
      cbn [fst snd].
      split; [exact Hok2|].
      destruct HPostExt2 as [j2 Hj2 HPostall2].
      econstructor; [exact Hj2|].
      intros j Hj.
      pose proof (HPostall2 j Hj) as Hpall2_j.
      destruct Hpall2_j as (HPre2_j & i_mid & Hi_mid & Hext_mid).
      split; [exact HPre2_j|].
      pose proof (HPostall1 i_mid Hi_mid) as Hpall1_mid.
      destruct Hpall1_mid as (_ & i_old & Hi_old & Hext_old).
      exists i_old; split; [exact Hi_old | eauto using map_extends_trans]. }
  Qed.
    ...
    { eapply state_sound_for_model_Mmap with
      ...
      - intros a.
        ...
        { eapply state_sound_for_model_strengthen_pre.
          - apply get_analysis_sound'.
          - intros i [_ HPi]; exact HPi. }
        intros r i HPi; split; [exact HPi | exact I].
      - intros i HPi; exact HPi.
      - intros b i i' Hext _; exact I. }
    intros r i [HPi _]; exact HPi.
  Qed.

  Lemma db_set_entry_sound' (Pre : idx_map (domain m) -> Prop) f args ep v an
    : (forall i, Pre i ->
  ...
  Proof.
    ...
    { eapply state_sound_for_model_bind.
      ...
      2:{
        ...
        { apply state_sound_for_model_ret. }
        intros r i HP.
        destruct HP as [HP1 _]. destruct HP1 as [HPi _]. exact HPi.
      }
      destruct e as [v_epoch v old_a]; cbn beta iota.
      (* Strengthen the Pre to also carry [atom_sound_for_model m i (Build_atom f args v)] *)
      pose (Pre' := fun i => Pre i /\
                  atom_sound_for_model m i
                    (Build_atom (atom_fn a) (atom_args a) v)).
      eapply state_sound_for_model_wkn_post.
      { eapply state_sound_for_model_strengthen_pre with (Pre1 := Pre').
        { eapply state_sound_for_model_bind.
          { apply (get_analyses_sound' Pre' (atom_args a)). }
          intros arg_as.
          case_match.
          ...
          { (* eqb out_a old_a = false: do the updates *)
            eapply state_sound_for_model_bind.
            { apply (update_analyses_sound' Pre'). }
            intros r1.
            eapply state_sound_for_model_bind.
            { apply (push_worklist_analysis_sound Pre'). }
            intros r2.
            apply (db_set_entry_sound' Pre').
            intros i HPi'. destruct HPi' as [_ Hatom]. exact Hatom. } }
        subst Pre'. cbv beta. intros i HP. exact HP. }
      subst Pre'. cbv beta. intros r i HP. destruct HP as [HPi _]; exact HPi. }
    cbv beta. intros r i HP. exact HP.
  Qed.
  ...
  Lemma list_Miter_repair_parent_analysis_sound (Pre : idx_map (domain m) -> Prop) ps
    : state_sound_for_model
        (fun i => Pre i /\ all (atom_sound_for_model m i) ps)
        (list_Miter repair_parent_analysis ps)
        (fun _ i => Pre i /\ all (atom_sound_for_model m i) ps).
  Proof.
    eapply state_sound_for_model_Miter with
      (P_inv := fun _ i => Pre i /\ all (atom_sound_for_model m i) ps).
    - intros a_atom.
      eapply state_sound_for_model_wkn_post.
      { eapply state_sound_for_model_strengthen_pre.
        { apply (repair_parent_analysis_sound
                   (fun i => In a_atom ps /\ Pre i /\ all (atom_sound_for_model m i) ps)
                   a_atom).
          intros i HP. destruct HP as [Hin HPi]. destruct HPi as [_ Hsound].
          eapply in_all; eauto. }
        intros i HP; exact HP. }
      intros r i HP. destruct HP as [_ HP]; exact HP.
    - intros i HP; exact HP.
  Qed.

  Lemma repair_after_mmap_sound (Pre : idx_map (domain m) -> Prop) x_canonical (improved : bool)
    : state_sound_for_model Pre
        (if improved
  ...
  Proof.
    destruct improved.
    - eapply state_sound_for_model_bind.
      { apply get_parents_sound. }
      intros canon_ps.
      eapply state_sound_for_model_wkn_post.
      { apply list_Miter_repair_parent_analysis_sound. }
      intros r i HP. destruct HP as [HPi _]; exact HPi.
    - eapply state_sound_for_model_wkn_post.
      { apply state_sound_for_model_ret. }
      intros r i HP. destruct HP as [HPi _]; exact HPi.
  ...
  Lemma repair_union_sound (Pre : idx_map (domain m) -> Prop) x_old x_canonical improved
    : state_sound_for_model Pre
  ...
  Proof.
    ...
    eapply state_triple_bind.
    { apply pull_parents_sound. }
    intros e0 old_ps e1 [Hok_e0 Hne_e0]
                       (Hok_e1 & (HPre1_atoms & Hain) & Hne1).
    enough (Hinner :
    ...
    { specialize (Hinner e1 eq_refl).
      destruct Hinner as (Hok_inner & HPre_inner & Hne_inner).
      split; [exact Hok_inner|].
      destruct HPre_inner as [j' Hj' HPpall].
      econstructor; [exact Hj'|].
      intros j Hj.
      split; [apply HPpall; exact Hj|].
      destruct Hne_inner as [_ _ Hne_all].
      apply (Hne_all j Hj). }
    eapply state_triple_bind.
    { eapply state_triple_consequence
        with (P' := fun e => egraph_ok e
                             /\ (forall_ne i | denote e i,
                                   Pre i /\ all (atom_sound_for_model m i) old_ps)
                             /\ all (fun a => atom_in_egraph_up_to_equiv a e) old_ps).
      - intros e He; subst e. exact (conj Hok_e1 (conj HPre1_atoms Hain)).
      - intros e0' p HP HQ; exact HQ.
      - apply list_Mmap_repair_each_sound. }
    intros e0' ps1 e2 He0' (Hok_e2 & HPre2 & Hne_map).
    cbn in He0'; subst e0'.
    pose proof (repair_after_mmap_sound Pre x_canonical improved e2
                  (conj Hok_e2 HPre2)) as Hcont.
    cbn beta in Hcont.
    destruct Hcont as (Hok_final & HPreFinalExt).
    split; [exact Hok_final|].
    destruct HPreFinalExt as [j' Hj' HPpall].
    split.
    { econstructor; [exact Hj'|].
      intros j Hj. apply (proj1 (HPpall j Hj)). }
    { econstructor; [exact Hj'|].
      intros j Hj.
      pose proof (HPpall j Hj) as Hpall_j.
      destruct Hpall_j as (_ & i_e2 & Hi_e2 & Hext_e2).
      destruct Hne_map as [_ _ Hne_map_all].
      pose proof (Hne_map_all i_e2 Hi_e2) as Hne_map_at.
      destruct Hne_map_at as (i_e1 & Hi_e1 & Hext_e1).
      destruct Hne1 as [_ _ Hne1_all].
      pose proof (Hne1_all i_e1 Hi_e1) as Hne1_at.
      destruct Hne1_at as (i_e0 & Hi_e0 & Hext_e0).
      exists i_e0; split; [exact Hi_e0|].
      eauto using map_extends_trans. }
  Qed.

  Lemma repair_sound (Pre : idx_map (domain m) -> Prop) a
    : state_sound_for_model Pre (repair a) (fun _ => Pre).
  Proof.
    destruct a as [old new improved | i_repair]; cbn [repair].
    - apply repair_union_sound.
    - eapply state_sound_for_model_bind.
      { apply get_parents_sound. }
      intros ps.
      eapply state_sound_for_model_wkn_post.
