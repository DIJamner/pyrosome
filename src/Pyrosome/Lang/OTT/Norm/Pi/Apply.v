...
Section Apply.
  ...
  | ap_app      : forall m s f vf a a' v,
      Apply_ne m s f vf -> Apply_val m s a a' -> Vapp m vf a' v ->
      Apply_ne m s (nApp f a) v
  | ap_appI     : forall m s f vf a a' v,
      Apply_ne m s f vf -> Apply_val m s a a' -> VappI m vf a' v ->
      Apply_ne m s (nAppI f a) v

  with Vapp : nat -> sval -> sval -> sval -> Type :=
  | vapp_lam : forall m b a v, Apply_val m (a :: id_list m) b v -> Vapp m (vLam b) a v
  | vapp_ne  : forall m n a, Vapp m (vNe n) a (vNe (nApp n a))

  with VappI : nat -> sval -> sval -> sval -> Type :=
  | vappI_lam : forall m b a v, Apply_val m (a :: id_list m) b v -> VappI m (vLamI b) a v
  | vappI_ne  : forall m n a, VappI m (vNe n) a (vNe (nAppI n a)).
...
Definition Apply_mutind
  ...
  (P3 : forall m vf a v, Vapp  m vf a v -> Type)
  (P4 : forall m vf a v, VappI m vf a v -> Type)
...
Lemma Apply_deterministic :
  ...
  * (forall m vf a v, Vapp m vf a v -> forall v'', Vapp m vf a v'' -> v = v'')
  * (forall m vf a v, VappI m vf a v -> forall v'', VappI m vf a v'' -> v = v'').
Proof.
  refine (Apply_mutind
    ...
    (fun m vf a v _ => forall v'', Vapp m vf a v'' -> v = v'')
    (fun m vf a v _ => forall v'', VappI m vf a v'' -> v = v'')
    ...
    | |- _ = ?rhs =>
        ...
        | H : Vapp  _ _ _ rhs |- _ => inversion H; subst; clear H
        | H : VappI _ _ _ rhs |- _ => inversion H; subst; clear H
        end
    end;
    ...
    | IH : forall v'', Vapp ?m ?vf ?a v'' -> _ = v'',
      H : Vapp ?m ?vf ?a _ |- _ => specialize (IH _ H); subst
    | IH : forall v'', VappI ?m ?vf ?a v'' -> _ = v'',
      H : VappI ?m ?vf ?a _ |- _ => specialize (IH _ H); subst
    end;
...
Definition Vapp_det  m vf a v v'' (H : Vapp m vf a v) := snd (fst Apply_deterministic) m vf a v H v''.
Definition VappI_det m vf a v v'' (H : VappI m vf a v) := snd Apply_deterministic m vf a v H v''.
