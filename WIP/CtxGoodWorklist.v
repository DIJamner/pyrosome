(* Scratch development of the P3 (I) assembly: ctx_readback_eF + ctx_readback_wf_subst.
   Imports the built Theorems.vo so all dependency lemmas are closed constants.
   Mirrors WithVar's context so transplant into Theorems.v is mechanical. *)
From Stdlib Require Import Lists.List Classes.RelationClasses BinNums.
From coqutil Require Import Map.Interface.
From Utils Require Import Utils Monad ExtraMaps VC UnionFind.
From Utils Require Import NamedList.
From Utils.EGraph Require Import Defs Semantics.
From Pyrosome.Theory Require Import Core.
From Pyrosome.Tools.EGraph Require Import Theorems.
From Utils.EGraph Require Import SemanticsRebuildCanon SemanticsParents SemanticsAnalysesCover SemanticsHashDb.
...
Section WithVar.
  ...
  Section AddCtxInvert.
    ...
    Lemma add_ctx_good_worklist c
      ...
      analyses_cover V V V_map V_map V_trie X e_in ->
      Defs.worklist e_in = [] ->
      ...
      /\ analyses_cover V V V_map V_map V_trie X (snd res)
