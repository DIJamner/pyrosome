...
From Named Require Import Core Exp BiRule.
Require Import String.

(* TODO: check_args needs to take in type data in term_by case*)
Inductive elab_sort (l : BiRule.lang) c : IExp.sort -> sort -> Prop :=
| elab_sort_by : forall n s es c' synths args,
    (n, (BiRule.sort_rule c' args)) \in l ->
    check_args l c synths args s es c' ->
    check_sort l c (IExp.srt n s) (Exp.srt n es)
with check_term (l : BiRule.lang) c : IExp.exp -> exp -> sort -> Prop :=
| check_term_by : forall n s es c' t synths args,
    (* TODO: arg order *)
    (n, (BiRule.term_rule c' synths args _ _ t)) \in l ->
    check_args l c synths args s es c' ->
    check_term l c (IExp.con n s) (con n es) t[/(with_names_from c' es)/]
...
| wf_term_var : forall n t,
    (n, t) \in c ->
    wf_term l c (var n) t
with check_args (l : BiRule.lang) c : list string -> list string -> list IExp.exp -> list exp -> ctx -> Prop :=.
                      wf_subst l c : subst -> ctx -> Prop :=
