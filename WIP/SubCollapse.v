...
(* ===== scratch: id/var-collapse builder ===== *)

(* Abbreviations for the wkn/hd instantiation (g:=wkn, D:=extG, a:=hd). *)
...
Proof.
  ...
  - eapply term_con_congruence.
    ...
    + cbn [with_names_from].
      eapply eq_args_cons.
      2:{ (* v-position: exp_subst snoc_a hdD = hd.
             Needs an annotation rewrite (ty_subst_cmp + El_subst_eq) to bring the
             cmp_snoc-produced annotation `ty_subst g0 (El F)` to snoc_hd's
             `ty_subst wknD (El act_code)`, then snoc_hd, then a sort conversion. *)
          admit. }
      eapply eq_args_cons.
      2:{ exact Hgeq. }
      eapply eq_args_refl.
