...
(* The 3-binder (relevant-domain) funext rule, checked against the SMALLEST
   prefix it needs: {Id, Id subst} ++ base.  It does not see the irrelevant
   funext rule; both are lifted into position by [Core.lang_ext_monotonicity]
   in IdCong.v. *)

Derive ott_id_funext_rel
       in (wf_lang_ext (ott_id_core ++ ott_id_base) ott_id_funext_rel)
       as ott_id_funext_rel_wf.
Proof.
  setup_lang_interactive.
  push_rule id_pi_pi_rel_rule.
...
#[local] Definition ott_id_funext_rel_entry := lang_entry ott_id_funext_rel_wf.
#[export] Hint Resolve ott_id_funext_rel_entry : wf_lang_db.
