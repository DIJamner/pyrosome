...
_─Scoped : Set ℓ → Set (suc (suc ℓ))
_─Scoped {ℓ} I = I → List I → Set (suc ℓ)
...
data Var {ℓ : Level} : {I : Set ℓ} → I ─Scoped where
  z : ∀[ (σ ∷_) ⊢ Var σ ]
  s : ∀[ Var σ ⇒ (τ ∷_) ⊢ Var σ ]
...
record Injective {I : Set ℓ₁} {J : Set ℓ₂} (f : I → J) : Set (ℓ₁ ⊔ ℓ₂) where
  constructor mkInjective
  field inj : ∀ {i₁ i₂} → f i₁ ≡ f i₂ → i₁ ≡ i₂
open Injective public

Injective-inj₁ : ∀ {A : Set ℓ₁} {B : Set ℓ₂} → Injective ((A → A ⊎ B) ∋ inj₁)
inj Injective-inj₁ refl = refl

Injective-inj₂ : ∀ {A : Set ℓ₁} {B : Set ℓ₂} → Injective ((B → A ⊎ B) ∋ inj₂)
inj Injective-inj₂ refl = refl
